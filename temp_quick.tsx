...
function QuickRecordInner({ quickMode, onQuickModeConsumed, ocrText }: { quickMode?: boolean; onQuickModeConsumed?: () => void; ocrText?: string }) {
  ...
  const handleImageUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      toast.error('AI 璇嗗埆澶辫触锛岃鎵嬪姩杈撳叆');
    } finally {
      setAiLoading(false);
    }
  }, [session]);
  ...
  const analyzeOcrText = async (text: string) => {
    ...
    try {
      ...
      if (data.result) {
        ...
        console.error('[OCR] API response:', data); toast.error(data.error || 'AI 识别失败，请手动输入'); console.log('[OCR] ocrText:', ocrText);
      }
    } catch {
      toast.error('缃戠粶閿欒锛岃閲嶈瘯');
    } finally {
      setAnalyzing(false);
    }
  };
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-10 pb-28">
        ...
        <div className="mb-3 flex items-center justify-between">
          <h1 className="text-[28px] font-semibold tracking-tight text-[#1D1D1F]">璁颁竴绗?/h1>
        ...
        {(aiLoading || analyzing) && (
          <div className="mb-3 rounded-xl bg-white p-3 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-[#86868B]">AI 姝ｅ湪鍒嗘瀽...</span>
            </div>
          </div>
        )}
        ...
        {aiResult && !aiLoading && !analyzing && (
          <div className="mb-3 flex items-center justify-between rounded-xl bg-[#F0FFF4] p-2.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-[#34C759]">AI 宸茶瘑鍒紝璇风‘璁?/span>
            </div>
        ...
        {recordType === 'business' && trips.length > 0 && (
          <div className="mb-3 rounded-xl bg-white p-3 shadow-sm">
            <label className="mb-1.5 block text-xs font-medium text-[#86868B]">鍑哄樊椤圭洰</label>
            <select
              value={tripId ?? ''}
        ...
        <div className="rounded-2xl bg-white shadow-sm">
          ...
          >
            <span className="text-xs font-medium text-[#86868B]">鏇村璇︽儏</span>
            {showDetails ? (
          ...
          {showDetails && (
            <div className="space-y-3 border-t border-[#E5E5EA] p-3">
              {/* Payment Method */}
              <div>
                <label className="mb-1 block text-xs font-medium text-[#86868B]">鏀粯鏂瑰紡</label>
              ...
              {/* Merchant */}
              <div>
                <label className="mb-1 block text-xs font-medium text-[#86868B]">鍟嗗</label>
                <Input
                  value={merchant}
                  onChange={(e) => setMerchant(e.target.value)}
                  placeholder="鍟嗗鍚嶇О锛堝彲閫夛級"
              ...
              <div className="flex gap-2">
                <div className="flex-1">
                  <label className="mb-1 block text-xs font-medium text-[#86868B]">鏃ユ湡</label>
                  <Input
                    type="date"
                    value={recordDate}
                ...
                <div className="flex-1">
                  <label className="mb-1 block text-xs font-medium text-[#86868B]">鏃堕棿</label>
                  <Input
                    type="time"
                    value={recordTime}
              ...
              {/* Note */}
              <div>
                <label className="mb-1 block text-xs font-medium text-[#86868B]">澶囨敞</label>
                <Input
                  value={note}
              ...
              {recordType === 'business' && (
                <div className="flex items-center justify-between">
                  <span className="text-sm text-[#1D1D1F]">鍙姤閿€</span>
                  <button
...
export function QuickRecord({ quickMode, onQuickModeConsumed, ocrText }: { quickMode?: boolean; onQuickModeConsumed?: () => void; ocrText?: string | null }) {
  return (
    <Suspense fallback={<div className="flex min-h-[50vh] items-center justify-center"><div className="text-sm text-[#86868B]">鍔犺浇涓?..</div></div>}>
